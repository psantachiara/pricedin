(function () {
  ...
  function setupTheme() {
    ...
    const sync = () => box.querySelectorAll('button').forEach((b) =>
      b.setAttribute('aria-pressed', String(b.dataset.themeValue === effective())));
  ...
  function init(config, rawEvents, prices, thumbs) {
    ...
    function renderDetail() {
      detail.innerHTML = '';
      detail.classList.remove('is-cluster');
      const sel = state.pinned;
      detail.hidden = !sel;
      if (!sel) return;
      const evs = sel.ids.map((id) => eventBy.get(id)).filter(Boolean);
      const clear = el('button', { type: 'button', class: 'text-button', text: 'Clear selection', onclick: unpin });
      ...
      if (evs.length === 1) {
        ...
        detail.append(el('a', {
          class: 'detail-media', href: e.url, target: '_blank', rel: 'noopener', tabindex: '-1', 'aria-hidden': 'true',
        }, thumbEl(e)));
        info.append(
        ...
        detail.classList.add('is-cluster');
